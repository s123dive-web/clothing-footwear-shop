      ...
      <div className="app" style={S.app}>
        ...
        <main className="main" style={S.main}>
          {!loaded ? (
            ...
            <BarcodeCreator items={items} notify={notify} log={addLog} />
          ) : tab === "sales" ? (
            <SalesHistory
              sales={sales}
              items={items}
...
// ---------- Barcode / Tag Creator ----------
// Format a YYYY-MM-DD date as MM/YY for compact tags.
const mmYY = (ds) => {
  if (!ds) return "";
  const [y, m] = ds.split("-");
...
function BarcodeCreator({ items, notify, log }) {
  const config = useShopConfig();
  ...
  useEffect(() => {
    ...
    try {
      ...
      setErr(
        "This value isn't valid for " +
          format +
          (format === "EAN13" ? " — EAN-13 needs 12–13 digits." : ".")
      );
    }
  }, [code, format]);
  ...
  const pickItem = (id) => {
    ...
    if (!it) return;
    setName(it.name);
    setMrp(it.mrp || it.sellingPrice || "");
    setSell(it.sellingPrice || "");
    setCode(it.code ? it.code : genCode(format));
  };

  const saveToItem = async () => {
    if (!itemId) return notify("Pick a product first to save its barcode");
    if (!code.trim()) return notify("Nothing to save");
    const it = items.find((i) => i.id === itemId);
    if (!it) return notify("That product no longer exists.");
    try {
      await cloudWriteProduct({ ...it, code: code.trim() });
      log("inventory", `Set barcode for “${name}” → ${code.trim()}`);
      notify("Barcode saved to product — it can now be scanned at billing");
    } catch (e) {
      console.error("save code failed", e);
      notify("⚠ Couldn't save the barcode.");
    }
  };
  ...
  const printLabels = () => {
    ...
    let url;
    try {
      url = barcodeDataUrl(code.trim(), format);
    } catch {
      return notify("Invalid barcode value for " + format);
    }
    const n = Math.max(1, Math.min(300, +qty || 1));
    const cur = CURRENCY.symbol;
    const priceLine = [
      mrp ? "MRP " + cur + escapeHtml(String(mrp)) : "",
      sell ? "Sell " + cur + escapeHtml(String(sell)) : "",
    ]
      .filter(Boolean)
      .join("&nbsp;&nbsp;");
    const dateLine = [pkd ? "PKD " + mmYY(pkd) : "", exp ? "EXP " + mmYY(exp) : ""]
      .filter(Boolean)
      .join("&nbsp;&nbsp;");
    const one = `<div class="lbl">
      <div class="store">${escapeHtml(config.name)}</div>
  ...
  return (
    <div>
      <Header
        title="Tag / Barcode"
        sub="Generate scannable tags to attach to garments & footwear"
      />
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <section style={S.panel}>
          <div style={S.panelHead}>Label details</div>

          <Field label="From products (optional)">
          ...
          <Field label="Product name">
            <input
              ...
              placeholder="e.g. Cotton Kurti — Maroon"
            />
          </Field>

          <div
            style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, alignItems: "end" }}
          >
            ...
            <button
              ...
              onClick={() => setCode(genCode(format))}
            >
              Generate
            </button>
          </div>
          ...
          <div
            style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: -2, marginBottom: 12 }}
          >
            <span style={{ fontSize: 11.5, color: "#6B7E74", alignSelf: "center" }}>
              Expiry quick-set:
            </span>
            {[
              ["1w", 7],
              ...
              <button key={lbl} className="btn small ghost" onClick={() => addExpiry(d)}>
                +{lbl}
              </button>
            ))}
          </div>
          ...
          {itemId && (
            <button
              ...
              onClick={saveToItem}
            >
              Save this barcode to the inventory item
            </button>
          )}
        </section>
        ...
        <section style={S.panel}>
          ...
          <div style={{ display: "grid", placeItems: "center", padding: "10px 0 16px" }}>
            ...
            >
              <div style={{ fontSize: 10, fontWeight: 800, color: "#8E2C48" }}>{config.name}</div>
              <div style={{ fontSize: 12.5, fontWeight: 700, lineHeight: 1.1 }}>
                {name || <span style={{ color: "#AAB" }}>Product name</span>}
              </div>
              <svg ref={svgRef} style={{ maxWidth: "100%" }} />
              {mrp || sell ? (
                <div style={{ fontSize: 12, fontWeight: 800 }}>
                  {mrp ? "MRP " + CURRENCY.symbol + mrp : ""}
                  {mrp && sell ? " · " : ""}
                  {sell ? "Sell " + CURRENCY.symbol + sell : ""}
                </div>
              ) : null}
          ...
          <div style={{ fontSize: 11.5, color: "#8A9C90", marginTop: 10, lineHeight: 1.5 }}>
            Labels print as a tiled sheet at true millimetre size with dashed cut guides — set your
            printer to 100% / “Actual size” (not “Fit”). Saving the barcode to an item lets the
            cashier scan it on the Billing screen.
          </div>
        </section>
      </div>
    </div>
